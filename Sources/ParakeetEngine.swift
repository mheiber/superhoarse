...
class ParakeetEngine: SpeechRecognitionEngine {
    ...
    func initialize() async throws {
        ...
        do {
            ...
            logger.info("Created ASRConfig for speech transcription")
    ...
    func transcribe(_ audioData: Data) async -> String? {
        ...
        var floatArray = convertAudioDataToFloatArray(audioData)
        ...
        // FluidAudio v3 requires minimum 1 second of audio (16000 samples at 16kHz)
        let minimumSamples = 16000
        if floatArray.count < minimumSamples {
            let durationSeconds = Double(floatArray.count) / 16000.0
            logger.info("Audio shorter than minimum (\(String(format: "%.2f", durationSeconds))s), padding to 1 second")
            floatArray.append(contentsOf: [Float](repeating: 0.0, count: minimumSamples - floatArray.count))
        }
