...
cleanup() {
    ...
    # Clean up build artifacts in project directory
    if [ ! -z "$PROJECT_BUILD_DIR" ] && [ -d "$PROJECT_BUILD_DIR" ]; then
        echo "Cleaning up project build directory..."
...
# Set volume to a reasonable level (20%) to ensure audio is audible
...
# 2. Build the app with Xcode in the project directory
echo "Building Superhoarse with Xcode (Release configuration)..."

# Create a dedicated build directory in the project
PROJECT_BUILD_DIR="./build"
rm -rf "$PROJECT_BUILD_DIR"
mkdir -p "$PROJECT_BUILD_DIR"

# Build using xcodebuild with explicit output directory
xcodebuild -project superhoarse.xcodeproj \
    -scheme superhoarse \
    -configuration Release \
    -derivedDataPath "$PROJECT_BUILD_DIR/DerivedData" \
    build \
...
# Find the built app in our project build directory
BUILT_APP=$(find "$PROJECT_BUILD_DIR" -name "superhoarse.app" -type d | head -1)

if [ -z "$BUILT_APP" ]; then
    echo "❌ Could not find built app in project build directory"
    echo "Searching for app in build directory..."
    find "$PROJECT_BUILD_DIR" -name "*.app" -type d
    exit 1
fi

echo "✅ Found built app at: $BUILT_APP"

# Sign the app for local execution
echo "Signing app for local execution..."
codesign --force --deep --sign - "$BUILT_APP" || echo "Warning: Could not sign app, proceeding anyway..."

# 3. Start the app
echo "Starting Superhoarse from built app..."

# Verify the executable exists
EXECUTABLE_PATH="$BUILT_APP/Contents/MacOS/superhoarse"
if [ ! -f "$EXECUTABLE_PATH" ]; then
    echo "❌ Executable not found at: $EXECUTABLE_PATH"
    echo "Contents of app bundle:"
    find "$BUILT_APP" -type f | head -10
    exit 1
fi
